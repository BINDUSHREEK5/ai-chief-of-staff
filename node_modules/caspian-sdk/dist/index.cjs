...
var WebhookVerificationError = class _WebhookVerificationError extends CommError {
  constructor(detail) {
    super(401, detail);
...
var EDIT_CHANNELS = /* @__PURE__ */ new Set(["telegram", "discord", "slack"]);
var Message = class {
  constructor(id, conversationId, connectionId, customerId, agentId, channel, sender, subject, text, html, client, media = [], chatType = null) {
    this.id = id;
...
var StreamResponse = class {
  constructor(client, messageId, channel, throttleMs = 500) {
    this.client = client;
  ...
  async sendIfNeeded() {
    ...
    if (this.outboundId === null) {
      ...
      this.outboundId = result.id ?? null;
      if (this.outboundId === null) {
  ...
  async editWithRetry(text) {
    const delays = [200, 400, 600, 800, 1e3];
    for (let attempt = 0; attempt < delays.length; attempt++) {
      try {
        await this.client.edit(this.outboundId, text);
        return;
      } catch (err) {
        if (err instanceof CommError && err.statusCode === 400 && err.detail === "Can only edit an outbound message that was sent") {
...
var WebhookDedup = class {
  constructor(maxSize = 1024, ttlMs = 3e5) {
...
`
  ...
  async handleWebhook(opts) {
    ...
    const hmacHex = (0, import_node_crypto.createHmac)("sha256", secret).update(raw).digest("hex");
    const expected = `sha256=${hmacHex}`;
    const sigBuf = Buffer.from(signature);
    const expBuf = Buffer.from(expected);
    if (sigBuf.length !== expBuf.length || !(0, import_node_crypto.timingSafeEqual)(sigBuf, expBuf)) {
